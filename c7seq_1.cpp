...
static void enum_test()
{
	...
	p_("-");
	{
	    ...
	    for (auto [i, s]: c7::seq::enumerate(sv, 1)) {
		p_("%{} %{}", i, s);
		s = "<" + s + ">";
	    };
	    for (auto& s: sv) {
	...
	p_("-");
	{
	    std::string sv[] = {"one", "two", "three"};
	    for (auto [i, s]: c7::seq::enumerate(c7::seq::c_array(sv), 1)) {
		p_("%{} %{}", i, s);
		s = "<" + s + ">";
	    };
	    for (auto& s: sv) {
...
static void conv_test()
{
    p_("-- convert [& enum] ------------------------------------------");
    {
	std::vector<int> vec{10, 11, 12, 13, 14};
	for (auto [i, p]: c7::seq::enumerate(c7::seq::head(c7::seq::ptr(vec), 3))) {
	    p_("%{} %{} %{}", i, p, *p);
	    *p += 100;
	}
	for (auto v: vec) {
	...
	for (auto [i, p]: c7::seq::tail(c7::seq::enumerate((c7::seq::ptr(vec))), 2)) {
	    p_("%{} %{} %{}", i, p, *p);
	}
    }
}
