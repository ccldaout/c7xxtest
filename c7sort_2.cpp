#define C7_MSORT_MT_THRESHOLD	1000
#define C7_QSORT_MT_THRESHOLD	1000
#define C7_RSORT_MT_THRESHOLD	1000
...
struct elm_t {
    elm_t() = default;
    elm_t(const elm_t&) = delete;
    elm_t& operator=(const elm_t&) = delete;
    elm_t(elm_t&& o): key(o.key), data(o.data) {
	o.key = 0;
	o.data = 0;
    }
    elm_t& operator=(elm_t&& o) {
	if (this != &o) {
	    key = o.key;
	    data = o.data;
	    o.key = 0;
...
static void copy_data(const elmvec_t& v1, elmvec_t& v2)
{
    v2.resize(v1.size());
    auto p1 = static_cast<const void*>(v1.data());
    auto p2 = static_cast<      void*>(v2.data());
    std::memcpy(p2, p1, v1.size()*sizeof(v1[0]));
}
...
static void testround(void (*make_data)(elmvec_t&))
{
    ...
    copy_data(BaseData, Ref1Data);
    std::sort(Ref1Data.begin(), Ref1Data.end());

    copy_data(BaseData, Ref2Data);
