...
class svr_service: public ext::attached<service_interface<my_msgbuf>> {
public:
    using base_type = service_interface<my_msgbuf>;
	...
	if (auto res = msgbuf.send(ext_attached); !res) {
	    c7echo(res);
	    for (auto& [pp, io_res]: ext_attached.errors()) {
    ...
    void on_message(monitor&, socket_port& port, my_msgbuf& msgbuf) override {
	p_("on_message: %{}, event:%{}, <%{}>",
	   port,
	   msgbuf.header.event,
    ...
    detach_id on_detached(monitor& mon, socket_port& port, provider_hint hint) override {
	p_("on_detached: %{}", port);
...
class cli_service: public service_interface<my_msgbuf> {
public:
    using base_type = service_interface<my_msgbuf>;
...
int main(int argc, char **argv)
{
	...
	c7::event::manage_acceptor(socket_port(std::move(svr_sock)),
				   [n]() mutable {
				       auto s = c7::format("%{}#%{}", "server", ++n);
				       return std::make_shared<svr_service>(s);
				   });
    } else {
	// client
